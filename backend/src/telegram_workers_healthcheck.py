...
"""
This script telegram workers use as healthcheck for docker-containers.
"""
...
active_accounts = {int(task['args'][1:-1].split(',')[0])
                   ...
                   if task['name'] == 'telegram.tasks.telegram_spawner'}

runned_accounts = TelegramAccount.objects.filter(runned=True)

for account in runned_accounts:
